...
public class PhoneCallListener extends PhoneStateListener {
    ...
    private long lastCallstateTimeStamp = 0L;
    ...
    public interface PhoneCallHandler {
        void handleMisCall(String name, String incomingNumber);

        void handleIncomingCall(String name, String incomingNumber);

        void handleHangUpCall(String incomingNumber);
    }
    ...
    public PhoneCallListener(Context context) {
        ...
        LogUtil.i(TAG, "PhoneCallListener init finish");
    }
    ...
    public void registerToTelephonyManager() {
        ...
        LogUtil.i(TAG, "PhoneCallListener registerToTelephonyManager");
    }

    public void unregisterFromTelephonyManager(){
        tm.listen(this, PhoneStateListener.LISTEN_NONE);
        LogUtil.i(TAG, "PhoneCallListener unregisterFromTelephonyManager");
    }

    @Override
    public void onCallStateChanged(int state, String incomingNumber) {
        super.onCallStateChanged(state, incomingNumber);
        LogUtil.i(TAG, "--->onCallStateChanged()");
        if (Math.abs(System.currentTimeMillis() - lastCallstateTimeStamp) < 500) {
            LogUtil.i(TAG, "state change interval too fast, Skip...");
            return;
        }
        lastCallstateTimeStamp = System.currentTimeMillis();
        LogUtil.i(TAG, "<<==state:" + state + "  " + lastCallNo);
        //未接号码
        if (lastetState == TelephonyManager.CALL_STATE_RINGING && state == TelephonyManager.CALL_STATE_IDLE) {
            final String name = getContactNameByNumber(context, incomingNumber);
            LogUtil.i(TAG, "：" + "未接号码:" + incomingNumber + "   " + name);
            lastetState = state;
            if (PushConfig.getMissCall()) {
                phoneCallHandler.handleMisCall(name, incomingNumber);
            }
        }
        lastetState = state;
        switch (state) {
            case TelephonyManager.CALL_STATE_RINGING:
                try {
                    ...
                    lastCallNo = "00000000000";
                }
                String name = "";
                name = getContactNameByNumber(context, lastCallNo);
                LogUtil.i(TAG, "===>>1name: " + name);
                if ((null == lastCallNo) || (lastCallNo.length() <= 2) || (!lastCallNo.matches("[0-9]+"))) {
                    lastCallNo = "00000000000";
                }
                if ((null != lastCallNo) && (lastCallNo.length() > 2)) {
                    LogUtil.i(TAG, "发送当前来电!");
                    if (PushConfig.getIncomingCall()) {
                        phoneCallHandler.handleIncomingCall(name, incomingNumber);
                    }
                }
                LogUtil.i(TAG, "<<==RINGING");
                break;
            //挂电话
            case TelephonyManager.CALL_STATE_IDLE:
                ...
                if (isFirstEntry) {
                    ...
                    if (PushConfig.getHangUpCall()) {
                        phoneCallHandler.handleHangUpCall(incomingNumber);
                    }
                    LogUtil.i(TAG, "被挂电话!隐藏图标");
                }
                LogUtil.i(TAG, "call notification     IDLE");
                break;
            case TelephonyManager.CALL_STATE_OFFHOOK:
                LogUtil.i(TAG, "call notification    OOFHOK");
                if (PushConfig.getHangUpCall()) {
                    phoneCallHandler.handleHangUpCall(incomingNumber);
                }
                LogUtil.i(TAG, "<<==OFFHOOK");
                break;
        }
    }
   ...
   判断国家区号，忽略+
    */
    private static boolean isCorrectCountryCode(final String code) {
        try {
