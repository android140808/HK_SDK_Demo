...
public class GetHeartRateCount extends BaseCommand {
    public GetHeartRateCount(CommandResultCallback resultCallback) {
        super(resultCallback, CommandConstant.COMMANDCODE_TOTAL_HEART_COUNT, CommandConstant.ACTION_CHECK);
    ...
    public int parse80BytesArray(int len, byte[] bytes) {
        ...
        GlobalVarManager.getInstance().setHeartRateCount(0);
        if (len == 2) {
            GlobalVarManager.getInstance().setHeartRateCount((int) ParseUtil.bytesToLong(bytes, 0, 1));
