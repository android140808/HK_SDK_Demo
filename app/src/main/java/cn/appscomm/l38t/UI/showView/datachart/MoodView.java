...
public class MoodView extends View {
    public static final int VIEW_TYPE_CALM = 90;
    public static final int VIEW_TYPE_MODERATE = 91;
    public static final int VIEW_TYPE_DEPRESSED = 92;
    ...
    private LinkedHashMap<Integer, MoodFatigueData> moodDatas;
    ...
    private int min_val = MoodFatigueData.MOOD_MIN, max_val = MoodFatigueData.MOOD_MAX + 1;
    ...
    public void initLength() {
        getViewTreeObserver().addOnGlobalLayoutListener(new ViewTreeObserver.OnGlobalLayoutListener() {
            ...
            public void onGlobalLayout() {
                ...
                if (isFirstDraw) {
                    if (moodDatas == null) {
                        moodDatas = new LinkedHashMap<Integer, MoodFatigueData>();
                    }
                    initTimeDatas();
                    init();
                    isFirstDraw = false;
    ...
    private void convertData2Point() {
        ...
        for (Integer index : moodDatas.keySet()) {
            MoodFatigueData bean = moodDatas.get(index);
            if (bean != null && bean.getMoodStatus() != -1) {
                int x = (int) (rateX * index + paddingLeft * 2);
                int y = (int) getDrawLineHeigh(bean.getMoodStatus());
                pointList.add(new Point(x, y, bean.getMoodStatus(),bean));
            }
        }
    }
...
//                postInvalidate();
    ...
    private int getValuePainColor(int value) {
        int color = getResources().getColor(R.color.mood_calm);
        switch (value) {
            case CALM:
                color = getResources().getColor(R.color.mood_calm);
                break;
            case MODERATE:
                color = getResources().getColor(R.color.mood_moderate);
                break;
            case DEPRESSED:
                color = getResources().getColor(R.color.mood_depressed);
                break;
        }
        return color;
    }
    ...
    private String getValueText(int value) {
        ...
        switch (value) {
            case CALM:
                valueS = getResources().getString(R.string.mood_calm);
                break;
            case MODERATE:
                valueS = getResources().getString(R.string.mood_moderate);
                break;
            case DEPRESSED:
                valueS = getResources().getString(R.string.mood_depressed);
                break;
        }
        return valueS;
    }
    ...
    public void setDatas(LinkedHashMap<Integer, MoodFatigueData> moodDatas) {
        if (this.moodDatas == null) {
            this.moodDatas = new LinkedHashMap<>();
        }
        this.moodDatas.clear();
        for (Integer key : moodDatas.keySet()) {
            this.moodDatas.put(key, moodDatas.get(key));
        }
        convertData2Point();
        invalidate();
    }
